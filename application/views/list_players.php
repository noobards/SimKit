				...
				<div class="table-mockup">
					...
					<div class="tbody">
						<div class="tr" ng-repeat="player in my_players | filter : searchKW as filtered">
							...
							<div class="td"><img class="role-icon" ng-src="<?php echo base_url(); ?>assets/images/icons/{{player.icon}}" title="{{player.player_type}}" alt="{{player.player_type}}" /><img class="role-icon" ng-src="<?php echo base_url(); ?>assets/images/icons/{{player.ment_icon}}" title="{{player.ment_label}}" alt="{{player.ment_label}}" /><a class="normal-anchor" href="Edit/{{player.id}}">{{player.name}}</a> <label class="label label-danger label-private-small" ng-show="player.is_private == 1">Private</label></div>							
